  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item v-if="flag=='yuangong'"  label="所属企业" prop="suoshuqiye">
          <el-select v-model="ruleForm.suoshuqiye" placeholder="请选择所属企业">
            <el-option
                v-for="(item,index) in yuangongsuoshuqiyeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  mounted() {
    ...
    this.$http({
      ...
      if (data && data.code === 0) {
        this.yuangongsuoshuqiyeOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
    ...
    this.yuangongzhiweiOptions = "员工,组长,副经理,总经理".split(',')
  },
  methods: {
