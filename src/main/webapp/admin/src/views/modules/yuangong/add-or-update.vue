<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="所属企业" prop="suoshuqiye">
          <el-select v-model="ruleForm.suoshuqiye" placeholder="请选择所属企业">
            <el-option
                v-for="(item,index) in suoshuqiyeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/qiyexinxi/qiyemingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.suoshuqiyeOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            ...
            this.zhiweiOptions = "员工,组长,副经理,总经理".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
